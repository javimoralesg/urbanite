...
/* Defines and enums ----------------------------------------------------------*/
#define TEST_PORT_FRONT_PARKING_SENSOR_ID 1 /*!< Ultrasound identifier @hideinitializer */

// Echo timer configuration
#define REAR_ECHO_TIMER TIM2                             /*!< Echo signal timer @hideinitializer */
#define REAR_ECHO_TIMER_CCMR_CCS_Pos TIM_CCMR2_CC3S_Pos  /*!< Echo signal timer capture/compare channel selection @hideinitializer */
#define REAR_ECHO_TIMER_CCMR_ICF TIM_CCMR2_IC3F          /*!< Echo signal timer input capture filter @hideinitializer */
#define REAR_ECHO_TIMER_CCMR_PSC TIM_CCMR2_IC3PSC        /*!< Echo signal timer input capture prescaler @hideinitializer */
#define REAR_ECHO_TIMER_CCER_CCP_Pos TIM_CCER_CC3P_Pos   /*!< Echo signal timer capture/compare channel positive polarity @hideinitializer */
#define REAR_ECHO_TIMER_CCER_CCNP_Pos TIM_CCER_CC3NP_Pos /*!< Echo signal timer capture/compare channel negative polarity @hideinitializer */
#define REAR_ECHO_TIMER_CCER_CCE TIM_CCER_CC3E           /*!< Echo signal timer capture/compare channel @hideinitializer */
#define REAR_ECHO_TIMER_DIER_CCIE TIM_DIER_CC3IE         /*!< Echo signal timer enable capture/compare channel interrupt @hideinitializer */
...
{
    UNITY_TEST_ASSERT_EQUAL_INT(GPIOA, STM32F4_FRONT_PARKING_SENSOR_ECHO_GPIO, __LINE__, "ERROR: STM32F4_FRONT_PARKING_SENSOR_ECHO_GPIO GPIO must be GPIOA");
    UNITY_TEST_ASSERT_EQUAL_INT(1, STM32F4_FRONT_PARKING_SENSOR_ECHO_PIN, __LINE__, "ERROR: STM32F4_FRONT_PARKING_SENSOR_ECHO_PIN pin must be 1");
}
...
void _test_regs_echo(void)
{
    ...
    uint32_t prev_gpio_mode = STM32F4_FRONT_PARKING_SENSOR_ECHO_GPIO->MODER;
    uint32_t prev_gpio_pupd = STM32F4_FRONT_PARKING_SENSOR_ECHO_GPIO->PUPDR;

    // Call configuration function
    port_ultrasound_init(TEST_PORT_FRONT_PARKING_SENSOR_ID);

    // Check that the mode is configured correctly
    uint32_t echo_mode = ((STM32F4_FRONT_PARKING_SENSOR_ECHO_GPIO->MODER) >> (STM32F4_FRONT_PARKING_SENSOR_ECHO_PIN * 2)) & GPIO_MODER_MODER0_Msk;
    ...
    uint32_t echo_pupd = ((STM32F4_FRONT_PARKING_SENSOR_ECHO_GPIO->PUPDR) >> (STM32F4_FRONT_PARKING_SENSOR_ECHO_PIN * 2)) & GPIO_PUPDR_PUPD0_Msk;
    ...
    uint32_t echo_af = ((STM32F4_FRONT_PARKING_SENSOR_ECHO_GPIO->AFR[STM32F4_FRONT_PARKING_SENSOR_ECHO_PIN / 8]) >> ((STM32F4_FRONT_PARKING_SENSOR_ECHO_PIN % 8) * 4)) & 0xF;
    ...
    uint32_t mask = ~(GPIO_MODER_MODER0_Msk << (STM32F4_FRONT_PARKING_SENSOR_ECHO_PIN * 2));
    uint32_t prev_gpio_mode_masked = prev_gpio_mode & mask;

    mask = ~(GPIO_PUPDR_PUPD0_Msk << (STM32F4_FRONT_PARKING_SENSOR_ECHO_PIN * 2));
    uint32_t prev_gpio_pupd_masked = prev_gpio_pupd & mask;

    uint32_t curr_gpio_mode_masked = STM32F4_FRONT_PARKING_SENSOR_ECHO_GPIO->MODER & mask;
    uint32_t curr_gpio_pupd_masked = STM32F4_FRONT_PARKING_SENSOR_ECHO_GPIO->PUPDR & mask;
...
void test_echo_timer_precison()
{
    ...
    uint32_t echo_init_tick = port_ultrasound_get_echo_init_tick(TEST_PORT_FRONT_PARKING_SENSOR_ID);
    ...
    uint32_t echo_end_tick = port_ultrasound_get_echo_end_tick(TEST_PORT_FRONT_PARKING_SENSOR_ID);
    ...
    uint32_t echo_overflows = port_ultrasound_get_echo_overflows(TEST_PORT_FRONT_PARKING_SENSOR_ID);
    ...
    bool echo_received = port_ultrasound_get_echo_received(TEST_PORT_FRONT_PARKING_SENSOR_ID);
...
void test_echo_port_generalization(void)
{
    ...
    stm32f4_ultrasound_set_new_echo_gpio(TEST_PORT_FRONT_PARKING_SENSOR_ID, p_expected_gpio_port, expected_gpio_pin);

    // TEST init function
    // Enable RCC for the specific GPIO
    if (STM32F4_FRONT_PARKING_SENSOR_ECHO_GPIO == GPIOA)
    {
        RCC->AHB1ENR |= RCC_AHB1ENR_GPIOAEN; /* GPIOA_CLK_ENABLE */
    }
    else if (STM32F4_FRONT_PARKING_SENSOR_ECHO_GPIO == GPIOB)
    {
        RCC->AHB1ENR |= RCC_AHB1ENR_GPIOBEN; /* GPIOB_CLK_ENABLE */
    }
    else if (STM32F4_FRONT_PARKING_SENSOR_ECHO_GPIO == GPIOC)
    {
        RCC->AHB1ENR |= RCC_AHB1ENR_GPIOCEN; /* GPIOC_CLK_ENABLE */
    }

    // Clean/set all configurations
    SYSCFG->EXTICR[STM32F4_FRONT_PARKING_SENSOR_ECHO_PIN / 4] = 0;
    EXTI->RTSR = 0;
    EXTI->FTSR = 0;
    EXTI->EMR = 0;
    EXTI->IMR = 0;

    STM32F4_FRONT_PARKING_SENSOR_ECHO_GPIO->MODER |= (STM32F4_GPIO_MODE_OUT << (STM32F4_FRONT_PARKING_SENSOR_ECHO_PIN * 2U));     // Wrong configuration to check that the GPIO is not being modified
    STM32F4_FRONT_PARKING_SENSOR_ECHO_GPIO->PUPDR |= (STM32F4_GPIO_PUPDR_PULLUP << (STM32F4_FRONT_PARKING_SENSOR_ECHO_PIN * 2U)); // Wrong configuration to check that the GPIO is not being modified
    ...
    uint32_t expected_gpio_mode = STM32F4_FRONT_PARKING_SENSOR_ECHO_GPIO->MODER;
    uint32_t expected_gpio_pupd = STM32F4_FRONT_PARKING_SENSOR_ECHO_GPIO->PUPDR;
    ...
    uint32_t curr_gpio_mode = STM32F4_FRONT_PARKING_SENSOR_ECHO_GPIO->MODER;
    uint32_t curr_gpio_pupd = STM32F4_FRONT_PARKING_SENSOR_ECHO_GPIO->PUPDR;
